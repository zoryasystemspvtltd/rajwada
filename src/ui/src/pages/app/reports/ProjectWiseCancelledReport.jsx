...
const ProjectWiseCancelledReport = () => {
    //const [companies, setCompanies] = useState([]);
    const [projects, setProjects] = useState([]);
    const [towers, setTowers] = useState([]);
    const [filters, setFilters] = useState({ projectId: '', towerId: '', startDate: '', endDate: '' });
    ...
    const handleSearch = async () => {
        try {
            ...
            const response = await api.projectWiseCancelledReport({ data: payload });
    ...
    const handleExportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'ProjectWiseCancelled Report');
        const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
        const blob = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        saveAs(blob, `ProjectWiseCancelledReport-${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    return (
        <>
            <div className="app-page-title">
                <div className="page-title-heading text-uppercase">Project Wise Cancelled Report</div>
            </div>
...
export default ProjectWiseCancelledReport;
