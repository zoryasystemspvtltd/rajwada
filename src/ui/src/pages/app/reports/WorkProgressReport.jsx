...
const WorkProgressReport = () => {
    ...
    const [filters, setFilters] = useState({ companyId: '', projectId: '', towerId: '' });
    ...
    const handleSearch = async () => {
        try {
            ...
            const response = await api.workProgressReport({ data: options });
    ...
    const handleExportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'WorkProgress Report');
        const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
        const blob = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        saveAs(blob, `WorkProgressReport-${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    return (
        <>
            <div className="app-page-title">
                <div className="page-title-heading text-uppercase">Work Progress Report</div>
            </div>
